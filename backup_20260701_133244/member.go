...
func apiDaftar(w http.ResponseWriter, r *http.Request) {
	...
	_, err = DB.Exec(
		"INSERT INTO users (username, password, role, scope, status_akun) VALUES (?, SHA2(?,256), 'member', 'member', 'pending')",
		username, password,
	)
	if err != nil {
	...
	//    dan catat di tabel files (kategori foto_profil) untuk halaman Inventaris
